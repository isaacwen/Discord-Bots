...
The game that is run through this bot is almost classic Uno, but with a couple differences. One such difference is a common enhancement that is added by players, where, after a player plays a +2 or +4 card, subsequent players can avoid having to draw by playing a card of the same value (+2 or +4, respectively), and the first player who cannot player a card that is of that value has to draw the sum of all cards that previous players would have drawn. For example, if 3 consecutive players played +2 cards, then the fourth player, if they don't have a +2 card to play, would have to draw 6 cards.
